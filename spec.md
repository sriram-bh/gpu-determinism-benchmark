...
This is a deliberate correction from an earlier version of this spec, which proposed expressing G* as an LF reactor program with one reactor per pipeline stage (Prefill, then one DecodeStep per decode step). That approach was abandoned because the ordering it enforced — Prefill before Decode 1 before Decode 2, etc. — is not a contribution of Lingua Franca at all; any correct autoregressive implementation enforces this ordering inherently, since each decode step has a real data dependency on the previous step's KV cache. LF's actual guarantee (reactions fire in a deterministic order based on declared dependencies) has no visibility into the thing this project actually needs to measure: the relative order of individual CUDA kernels *within* a single forward pass, which is governed entirely by the GPU's hardware scheduler, a closed-source system no software-level language can reach into or control.
...
LF does not generate, define, or validate G*. Its role in this project is narrower and more concrete: coordinating the **hybrid kernel policy's** internal control logic. The hybrid policy (deterministic mode early in decode, throughput-optimized mode later, switching at some cache-size threshold) requires a mode-switch decision to be read and acted on consistently before each decode step's kernels are dispatched. Implemented naively in Python, this is vulnerable to a real race condition: CUDA kernel launches are asynchronous, so the host thread can race ahead and dispatch a decode step's kernels using a stale or inconsistently-read mode flag, especially under multi-stream configurations. Expressing the mode-switch logic as an LF reactor — which only fires once its declared dependency (e.g. a `cache_size_updated` event) has a definite value, and which guarantees deterministic ordering relative to the kernel-dispatching logic that depends on it — eliminates this race by construction, the same mechanism Lee's drive-by-wire example uses to guarantee brake-pressed state is resolved before torque is set. This does not give LF any control over GPU hardware scheduling; it guarantees the *policy decision itself* is never applied inconsistently.
